...
class Comm_HttpRequest {
    ...
    /**
     * 设置认证方式为Mauth(只支持GET请求)
     *  @param bool $mauth
     *  @return
     */
    public function set_mauth($mauth = true){
    	$this->mauth = (bool) $mauth;
    }
    
    /**
     * 设置认证方式为Tauth2(只支持POST请求)
     *  @param bool $tauth2
     *  @return
     */
   	public function set_tauth2($uid,$tauth2 = true){
   		if($uid && $tauth2){
   			//获取auth_header信息
   			$tauth = new Comm_Tauth($uid);
   			$auth_header = $tauth->getAuthorizationHeader();
   			$this->add_arr_header($auth_header);
   		}
   	}
    ...
    public function set_need_new_curl($flag = false) {
        if($flag) {
            $this->new_curl_pool = true;
        }
    }
    ...
    public function add_arr_header(Array $header, $urlencode = false) {
    	foreach ($header as $primary=>$secondary){
    		$this->add_header($primary, $secondary, $urlencode);
    	}
    }
    ...
    public function curl_init() {
        ...
        $ch = Comm_HttpRequestPool::get_curl($this->get_host_id(), $this->new_curl_pool);
    ...
    private function curl_setopt() {
        ...
        if (Comm_Context::get('isTauth2', false) === true && strpos($this->url, 'update.api.weibo.com') !== false) {
            Tool_Log_Commlog::write_log('TIMEWEIBO_TAUTH2', 'cookies:' . json_encode($this->cookies) . 'headers:' . json_encode($this->headers));
            unset($this->cookies);
            unset($_COOKIE);
        }
        ...
        if (Comm_Context::get('isTauth2', false) === true && strpos($this->url, 'update.api.weibo.com') !== false) {
            Tool_Log_Commlog::write_log('TIMEWEIBO_TAUTH2', 'curl_cli:' . json_encode($this->curl_cli));
        }
    }
    ...
    private function load_headers() {
    	if($this->mauth){
    		$signa_param   = $this->method == 'POST' ? $this->post_fields : $this->query_fields;
    		$this->headers = Comm_Mauth::getSignatrueByUrl($this->http_build_query($signa_param));
    	}
    ...
    private function load_query_fields() {
        ...
        if($this->mauth && $this->method == 'GET'){
        	$this->query_fields += Comm_Mauth::getMauthArr();
        }
    ...
    private function load_post_fields() {
        ...
        if($this->mauth && $this->method == 'POST'){
        	$this->post_fields += Comm_Mauth::getMauthArr();
        }
